...
import java.util.Map;
    ...
    int ballSize = 6;
    Map<Integer, Skeleton> skels = new Hashtable<Integer, Skeleton>();


    public void setup() {
        size((int) (screen.height * 4 / 3 / 1.1), (int) (screen.height / 1.1), OPENGL); //Keep 4/3 aspect ratio, since it matches the kinect's.
    ...
    float[] vertexFromJoint(float[] jointValues) {
        return new float[]{jointValues[0] * width, jointValues[1] * height, -jointValues[2] * 300};
    }

    float distanceBetweenJoints(float[] j1, float[] j2) {
        return sqrt(sq(j1[0] - j2[0]) + sq(j1[1] - j2[1]) + sq(j1[2] - j2[2]));
    }
    ...
    public void draw() {
        ...
        if(totalMovement > 2200) {
            fill(255,127,36);
        }

        if(totalMovement > 3300) {
            fill(255, 0, 0);
        }

        text("                      Score: " + round(totalMovement), 0, 0);
